...
std::list<mesh_t*>* loadMSH(const char* fileName)
{
	...
	if (result)
	{
		//not a polygon anymore, now it will be a mesh loaded
			...
			bufferNode = bufferNode.next_sibling())
		{
			...
			pugi::xml_node materialNode = bufferNode.child("material").child("texture");
			...
			//there might be no normals info in our file
			if (normals->size() != 0)
